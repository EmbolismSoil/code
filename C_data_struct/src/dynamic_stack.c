...
#define BLOCK_IS_EMPTY(_ptr) \
	((_ptr)->mlist_head->size >= BLOCK_SIZE ? 1 : 0)
#define BLOCK_IS_FULL(_ptr)  \
	((_ptr)->mlist_head->size <= 0 ? 1 : 0) 
#define PRE_BLOCK_IS_NULL(_ptr) \
	(!((_ptr)->mlist_head->pre)) 
#define PUSH(_ptr,_date) \
	push(_ptr, _date, sizeof(_date))
#define POP(_ptr,_buf) \
	pop(_ptr, _buf, sizeof(_buf))
...
*/
int add_block(stack *cosnt ptr, int size)
{
...
*/
int push(stack *const ptr, void *const date, int len)
{	
    int count;
...
int pop(stack *const ptr, void *const buf, int len)
{
    int count;
